...
const JobApplications = () => {
  ...
  const { data, error, mutate } = useSWR(`get-job-${jobId}`, () =>
    api.get(`/api/myjobs/${jobId}`)
  );
  ...
  const upgrade = async applicationId => {
    setLoading(true);

    try {
      if (window.confirm('Are you sure you want to upgrade the status?')) {
        await api.put(`/api/jobs/application/${applicationId}/upgrade`);
        await mutate();
      }
    } catch (err) {
  ...
  const reject = async applicationId => {
    ...
    try {
      if (window.confirm('Are you sure you want to reject this application?')) {
        await api.put(`/api/jobs/application/${applicationId}/reject`);
        await mutate();
      }
    } catch (err) {
      if (err.errors) dispatch(setError(err.errors[0].msg));
  ...
  return (
    <>
      ...
      <DataGrid
        ...
        rows={formattedData}
        columns={getColumns(upgrade, reject)}
        onCellClick={({ field, value }) => {
          if (field === 'Skills' || field === 'Education' || field === 'sop')
            window.alert(value);
        }}
      />
    </>
